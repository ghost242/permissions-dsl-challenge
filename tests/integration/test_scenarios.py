...
def setup_base_data(test_client):
    ...
    cursor.execute(
        ...
        ("creator1", "creator@example.com", "Creator"),
    )
    cursor.execute(
        "INSERT INTO users (id, email, name) VALUES (?, ?, ?)",
    ...
    cursor.execute(
        "INSERT INTO users (id, email, name) VALUES (?, ?, ?)",
        ("viewer1", "viewer@example.com", "Viewer"),
    )
    cursor.execute(
        "INSERT INTO users (id, email, name) VALUES (?, ?, ?)",
        ("stranger", "stranger@example.com", "Stranger"),
    )
...
class TestScenario2TeamAdminAccess:
    ...
    def test_scenario_2_team_admin_has_access(self, test_client):
        ...
        cursor.execute(
            "INSERT INTO resource_policies (resource_id, policy_document) VALUES (?, ?)",
            ("urn:resource:team1:proj1:doc2", policy_json),
        )
        test_client.test_db.commit()
        ...
        for action in ["can_view", "can_edit", "can_delete", "can_share"]:
            ...
            assert (
                response.json()["allowed"] is True
            ), f"Team admin should have {action}"
...
class TestScenario3ProjectMemberAccess:
    ...
    def test_scenario_3_project_member_has_access(self, test_client):
        ...
        cursor.execute(
            ...
            ("editor1", "proj1", "editor"),
        )
        cursor.execute(
            "INSERT INTO project_memberships (user_id, project_id, role) VALUES (?, ?, ?)",
            ("viewer1", "proj1", "viewer"),
        )
        ...
        cursor.execute(
            "INSERT INTO resource_policies (resource_id, policy_document) VALUES (?, ?)",
            ("urn:resource:team1:proj1:doc3", policy_json),
        )
        test_client.test_db.commit()
...
class TestScenario4PublicLinkAccess:
    ...
    def test_scenario_4_public_link_enabled(self, test_client):
        ...
        policy_json = json.dumps(
            {
                "resource": {
                    "resourceId": "urn:resource:team1:proj1:doc4",
                    "creatorId": "creator1",
                },
                "policies": [
                    {
        ...
        cursor.execute(
            "INSERT INTO resource_policies (resource_id, policy_document) VALUES (?, ?)",
            ("urn:resource:team1:proj1:doc4", policy_json),
        )
        test_client.test_db.commit()
...
class TestScenario5DeletedDocumentDenied:
    ...
    def test_scenario_5_deleted_document_denied(self, test_client):
        ...
        policy_json = json.dumps(
            {
                "resource": {
                    "resourceId": "urn:resource:team1:proj1:doc5",
                    "creatorId": "creator1",
                },
                "policies": [
                    {
        ...
        cursor.execute(
            "INSERT INTO resource_policies (resource_id, policy_document) VALUES (?, ?)",
            ("urn:resource:team1:proj1:doc5", policy_json),
        )
        test_client.test_db.commit()
        ...
        for action in ["can_view", "can_edit", "can_delete", "can_share"]:
            ...
            assert (
                response.json()["allowed"] is False
            ), f"Deleted doc should deny {action}"
...
class TestScenario6ExplicitDenyOverride:
    ...
    def test_scenario_6_explicit_deny_overrides_allow(self, test_client):
        ...
        policy_json = json.dumps(
            {
                "resource": {
                    "resourceId": "urn:resource:team1:proj1:doc6",
                    "creatorId": "creator1",
                },
                "policies": [
                    {
        ...
        cursor.execute(
            "INSERT INTO resource_policies (resource_id, policy_document) VALUES (?, ?)",
            ("urn:resource:team1:proj1:doc6", policy_json),
        )
        test_client.test_db.commit()
        ...
        assert (
            response.json()["allowed"] is False
        ), "Edit should be denied (DENY > ALLOW)"
...
class TestScenario7DefaultDeny:
    ...
    def test_scenario_7_no_permission_default_deny(self, test_client):
        ...
        policy_json = json.dumps(
            {
                "resource": {
                    "resourceId": "urn:resource:team1:proj1:doc7",
                    "creatorId": "creator1",
                },
                "policies": [
                    {
        ...
        cursor.execute(
            "INSERT INTO resource_policies (resource_id, policy_document) VALUES (?, ?)",
            ("urn:resource:team1:proj1:doc7", policy_json),
        )
        test_client.test_db.commit()
        ...
        for action in ["can_view", "can_edit", "can_delete", "can_share"]:
            ...
            assert (
                response.json()["allowed"] is False
            ), f"Stranger should not have {action} (default DENY)"
